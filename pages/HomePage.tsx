    ...
    <div className="space-y-12">
      ...
      {featuredPost && (
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Main Headline (2/3 width on desktop) */}
          <div className="lg:col-span-2 group relative h-[400px] lg:h-[500px] rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-500">
             <Link to={`/post/${featuredPost.slug}`} className="block h-full w-full">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end p-6 md:p-8">
                    <span 
                        className="inline-block px-3 py-1 mb-3 text-xs font-bold uppercase tracking-wider text-white rounded w-fit"
                        style={{ backgroundColor: theme.primaryColor }}
                    >
                        {featuredPost.tags[0] || 'News'}
                    </span>
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-2 leading-tight drop-shadow-sm">
                        {featuredPost.title}
                    </h2>
                    <p className="text-slate-200 line-clamp-2 md:text-lg max-w-2xl mb-2">
          ...
          {/* Side Columns (1/3 width, stacked) */}
                 ...
                 <Link key={post.id} to={`/post/${post.slug}`} className="flex-1 relative rounded-2xl overflow-hidden group shadow-sm hover:shadow-md transition-all">
                    {post.coverImage ? (
                     ...
                     <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-5">
                        ...
                        <h3 className="text-xl font-bold text-white leading-snug group-hover:underline decoration-2 underline-offset-4">
                            {post.title}
                        </h3>
             ...
             <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <article key={post.id} className="flex flex-col bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-slate-100 group">
                    <Link to={`/post/${post.slug}`} className="block h-48 overflow-hidden relative">
                        ...
                        <span className="absolute top-4 left-4 bg-white/90 backdrop-blur px-2 py-1 text-xs font-bold rounded text-slate-900 shadow-sm uppercase">
                            {post.tags[0]}
                        </span>
                    </Link>
                    ...
                    <div className="mt-auto pt-4 border-t border-slate-100 flex justify-between items-center text-xs text-slate-500 font-medium">
                        ...
                        <span className="flex items-center gap-1 text-[var(--secondary-color)]">
      ...
      {posts.length === 0 && (
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-slate-300">
          ...
          <Link to="/admin" className="text-indigo-600 hover:underline mt-2 inline-block">Go to Admin to generate content</Link>
        </div>
      )}
    </div>
  );
